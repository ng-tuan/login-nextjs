...
const DetailsPage: React.FC<DetailsPageProps> = ({ params }) => {
  ...
  const getColorClass = (colorName: string) => {
    ...
    const colorClassMap: Record<string, string> = {
      ...
      đen: 'bg-black',
      vàng: 'bg-amber-400',
      nâuđỏ: 'bg-red-800', // No space or hyphen in the original name
      // Add more color mappings as needed
    };

    return colorClassMap[formattedColorName] || 'bg-gray-200';
  };
  ...
  const getSizeClass = (sizeName: string) => {
    // Add size-to-class mapping as needed
    ...
    return sizeClassMap[sizeName] || 'text-base'; // Default to a base size class if not found
  };
  ...
  const handleAddToCart = (selectedVariation: {
    ...
    console.log('Adding to Cart:', selectedVariation);
                      ...
                      {getColorOptions().map((colorOption) => (
                        ...
                        >
                          ...
                          <span
                            aria-hidden="true"
                            className={classNames(
                              'h-8 w-8 rounded-full border border-opacity-10',
                              getColorClass(colorOption), // Use the getColorClass function here
                            )}
                          />
                        </RadioGroup.Option>
                      ))}
                    ...
                    <div className="grid grid-cols-4 gap-4 sm:grid-cols-8 lg:grid-cols-4">
                      {getSizeOptions().map((sizeOption) => (
                        <RadioGroup.Option
                          key={sizeOption}
                          value={sizeOption}
                          disabled={!sizeOption.inStock} // Use your logic to determine if the size is in stock
                          className={({ active }) =>
                            classNames(
                              sizeOption.inStock
                                ? 'cursor-pointer bg-white text-gray-900 shadow-sm'
                      ...
                      {getSizeOptions().map((sizeOption) => {
                        ...
                        return (
                          ...
                          <RadioGroup.Option
                            key={sizeOption}
                            value={sizeOption}
                            disabled={false} // Allow selection of all sizes
                            className={({ checked }) =>
                              classNames(
                                'cursor-pointer bg-white text-gray-900 shadow-sm',
                                checked ? 'ring-2 ring-indigo-500' : '', // Apply ring style only when checked
                                'group relative flex items-center justify-center rounded-md border py-3 px-4 text-sm font-medium uppercase hover:bg-gray-50 focus:outline-none sm:flex-1 sm:py-6',
                              )
                            }
                          >
                            <RadioGroup.Label
                              as="span"
                              //   className={getSizeClass(sizeOption)}
                            >
                              {sizeOption}
                            </RadioGroup.Label>
                            {isSizeInStock ? (
